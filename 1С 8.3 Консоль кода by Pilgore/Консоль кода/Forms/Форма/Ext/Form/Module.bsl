...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтображатьБыстрыеПодсказки = Истина;
	ОтображатьКартуКода = Истина;
...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.ПолеHTML.Документ.defaultView.style = "line-height: 30px !important";
КонецПроцедуры
...
&НаКлиенте
Процедура ПриГотовностиКонсоли(Элемент)
...
&НаКлиенте
Процедура ПриНажатииНаКонсоль(Элемент, ДанныеСобытия, СтандартнаяОбработка)
...
Процедура АктивироватьДополнительныеФункции()
	...
	ОбновитьОтображениеКартыКода(Неопределено);
	ОбновитьОтображениеСтрокиСостояния(неопределено);
	ОбновитьОтображениеПробелов(Неопределено);
...
&НаКлиенте
Процедура ОчиститьМетаданные() Экспорт
...
Процедура ИнициализацияРедактора()
	...
	View().hideScrollX();
	View().hideScrollY();
	View().setFontFamily("Fira Code, Consolas, monospace");
	View().setFontSize(18);
	View().setLineHeight("32px");
	
	ПодключитьОбработчикОжидания("ОчиститьМетаданные", 1, Истина);
...
&НаКлиенте
Процедура ОбновитьТему(Команда)
...
Процедура СохранитьКак(Команда)
	...
	Если ЕстьКод() Тогда
		ВыполнитьВыборФайлаИСохранение(Ложь);
	Иначе
		ПредупреждениеАсинх("Нет кода для сохранения!",,"Предупреждение");
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьЯзыкПодсказок(Команда)
	View().switchLang();
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьОтображениеПодсказок(Команда)
	View().enableQuickSuggestions(ОтображатьБыстрыеПодсказки);
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПодсветкуЗапросов(Команда)
...
&НаКлиенте
Процедура ОбновитьОтображениеКартыКода(Команда)
	View().minimap(ОтображатьКартуКода);
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьОтображениеСтрокиСостояния(Команда)
	
	Если ОтображатьСтрокуСостояния Тогда
		View().showStatusBar();
	Иначе
		View().hideStatusBar();
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьОтображениеПробелов(Команда)
...
&НаКлиентеНаСервереБезКонтекста функция ТемаСветлая() возврат 0; КонецФункции
&НаКлиентеНаСервереБезКонтекста функция ТемаТемная () возврат 1; КонецФункции

&НаКлиенте
Функция ПолучитьИмяТемы()
	
	Имена = Новый Соответствие();
	Имена.Вставить(ТемаСветлая(), "bsl-white");
	Имена.Вставить(ТемаТемная(), "bsl-dark");
	
	Для каждого тема Из Имена Цикл
		
		Если ТекущаяТема = тема.Ключ Тогда
			возврат тема.Значение;
		КонецЕсли;
...
&НаКлиенте
Процедура ПриИзмененииТекущейТемы(Элемент)
	Если ТекущаяТема = 0 Тогда      // Светлая тема
		View().setTheme("bsl-white");
	ИначеЕсли ТекущаяТема = 1 тогда // Темная тема
		View().setTheme("bsl-dark");
	КонецЕсли;
КонецПроцедуры
